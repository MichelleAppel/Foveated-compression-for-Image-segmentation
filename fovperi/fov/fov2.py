...
def reverseMapPixel( N, p0, Nr, bio=False ):
	...
	rev = np.load( './fovperi/fov/' + conf_str + '-rev' + bio_str + '.npy' )
...
def generate_fovea( N, p0, Nr ):
	...
	try:
		mX = np.load('./fovperi/fov/' + conf_str + '-mappingX.npy')
		mY = np.load('./fovperi/fov/' + conf_str + '-mappingY.npy')
	except FileNotFoundError:
		# If files don't exist, initialize your mappings here
		# This is just a placeholder; you might need a different initialization
		dims = 2 * (p0 + Nr) + 1
		mX = np.zeros((dims, dims))
		mY = np.zeros((dims, dims))
	...
	np.save( './fovperi/fov/' + conf_str + '-mappingX_bio.npy', mX )
	np.save( './fovperi/fov/' + conf_str + '-mappingY_bio.npy', mY )
	np.save( './fovperi/fov/' + str(dims) + 'x' + str(dims) + '_radial_normalized_bio.npy', (normalized*255).astype('int32')/255 )
	np.save( './fovperi/fov/' + str(dims) + 'x' + str(dims) + '_radial_0-255_bio.npy', (normalized*255).astype('int32') )
	np.save( './fovperi/fov/' + conf_str + '-rev_bio.npy', rev )
...
def add_dist_channel( batch_size, fovSize, bio=False ):
	...
	template = np.load( './fovperi/fov/81x81_radial_normalized' + bio_str + '.npy' )
...
def getFov( N, p0, Nr, bio=False, add_coords=False ):
	...
	mappingX = np.load( './fovperi/fov/' + conf_str + '-mappingX' + bio_str + '.npy' )
	mappingY = np.load( './fovperi/fov/' + conf_str + '-mappingY' + bio_str + '.npy' )

	maxShift = np.load( './fovperi/fov/' + conf_str + '-maxShift.npy' )
	minShift = -maxShift
	...
	def process( im, fovX=0, fovY=0, for_training=True, sample_matrix=False, add_coords=False, reverse=False ):
		...
		if reverse:
			...
			orig_reconst_interpolated = cv2.inpaint(np.uint8(orig_reconst), mask, 3, cv2.INPAINT_NS)
